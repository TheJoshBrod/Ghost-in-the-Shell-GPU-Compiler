...
#include <cuda_fp16.h>       // Required for __half type and related intrinsics (e.g., __half2float)
#include <c10/util/Half.h>  // Required for c10::Half (PyTorch's Half type)
...
torch::Tensor launch_minimum(torch::Tensor A, torch::Tensor B) {
    ...
    if (A.scalar_type() == torch::kFloat32) {
        ...
        // For PyTorch's kHalf, we use c10::Half which is an alias for __half on CUDA.
