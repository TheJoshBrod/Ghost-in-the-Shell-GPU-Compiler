...
__global__ void sub_kernel(const T* input1, const T* input2, T* output, int64_t n_elements, double alpha_scalar_double) {
    ...
    for (int64_t i = idx; i < n_elements; i += stride) {
        ...
        if (std::is_same<T, c10::Half>::value) {
            ...
            // Fallback for other integer types if needed, though problem description implies float/half/double
            // For integer types, it's common to cast to float/double for intermediate alpha multiplication
            // or ensure alpha is an integer. For now, this branch is not explicitly handled in the problem.
            // If T is an integer type, alpha_scalar_double being a double would imply a float/double cast.
            // Example for integer type:
            // output[i] = static_cast<T>(input1[i] - static_cast<T>(alpha_scalar_double * input2[i]));
            // However, the AT_DISPATCH_FLOATING_TYPES_AND_HALF only dispatches for floating point and half.
        }
    }
}
...
// Changed the alpha parameter type from torch::Scalar to double
torch::Tensor launch_sub(torch::Tensor input1, torch::Tensor input2, double alpha_val) {
    // --- Input Checks ---
    ...
    // alpha_val is already a double, no need to convert from torch::Scalar
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "sub_kernel", [&] {
        if (num_blocks > 0) { // Only launch kernel if there are elements to process
            sub_kernel<scalar_t><<<num_blocks, threads_per_block>>>(
                ...
                n_elements,
                alpha_val // Pass the double alpha_val directly
            );
        }
    });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    // The original PyTorch aten::sub operator has a default alpha=1.0.
    // The Python call was not providing 'alpha', leading to the argument mismatch.
    // By adding `py::arg("alpha") = 1.0`, we make 'alpha' an optional argument
    // that defaults to 1.0, matching PyTorch's behavior for `aten.sub`.
    // The C++ function now expects a double, which pybind11 handles smoothly
    // for python float/int and the default C++ double value.
    m.def("launch", &launch_sub, "Element-wise subtraction: output = input1 - alpha * input2",
          py::arg("input1"), py::arg("input2"), py::arg("alpha") = 1.0);
}
// [END kernel.cu]