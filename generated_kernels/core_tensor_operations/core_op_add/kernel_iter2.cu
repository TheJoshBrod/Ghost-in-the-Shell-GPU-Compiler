...
torch::Tensor launch_add(torch::Tensor A, torch::Tensor B) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(A.scalar_type(), "add_kernel", [&] {
        add_kernel<scalar_t><<<num_blocks, threads_per_block>>>(
            A.data_ptr<scalar_t>(), B.data_ptr<scalar_t>(), C.data_ptr<scalar_t>(), num_elements
        );
    });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    // The verifier expects the function to be named "launch"
    m.def("launch", &launch_add, "Element-wise addition of two tensors (CUDA)");
}
// [END kernel.cu]