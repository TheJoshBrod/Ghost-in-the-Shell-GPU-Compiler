...
#include <torch/extension.h>    // Include PyTorch extension header here for general utility
...
// The optional min/max arguments are now `c10::optional<double>` for pybind11 compatibility.
torch::Tensor launch_clamp(
    torch::Tensor self,
    c10::optional<double> min_opt_d, // Changed to c10::optional<double> for pybind11
    c10::optional<double> max_opt_d  // Changed to c10::optional<double> for pybind11
) {
    ...
    bool has_min = min_opt_d.has_value();
    double min_val_d = 0.0;
    if (has_min) {
        // Direct value extraction
        min_val_d = min_opt_d.value();
    }

    bool has_max = max_opt_d.has_value();
    double max_val_d = 0.0;
    if (has_max) {
        // Direct value extraction
        max_val_d = max_opt_d.value();
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    // Pybind11 automatically handles Python numbers to double and Python None to c10::optional null.
