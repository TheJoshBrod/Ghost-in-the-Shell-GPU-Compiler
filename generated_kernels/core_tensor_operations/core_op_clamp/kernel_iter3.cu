...
// The optional min/max arguments are now `c10::optional<at::Scalar>` to allow direct Python number passing.
torch::Tensor launch_clamp(
    torch::Tensor self,
    c10::optional<at::Scalar> min_opt, // Reverted to c10::optional<at::Scalar>
    c10::optional<at::Scalar> max_opt  // Reverted to c10::optional<at::Scalar>
) {
    ...
    if (has_min) {
        // Extract scalar value from c10::Scalar using toDouble()
        min_val_d = min_opt.value().toDouble();
    }
    ...
    if (has_max) {
        // Extract scalar value from c10::Scalar using toDouble()
        max_val_d = max_opt.value().toDouble();
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    // Pybind11 automatically handles Python numbers to at::Scalar and Python None to c10::optional null.
