...
torch::Tensor launch_pow(torch::Tensor base, torch::Tensor exp) {
    ...
    if (base.scalar_type() == torch::kFloat32) {
        ...
        // Corrected: Use c10::Half for data_ptr with PyTorch tensors.
        // c10::Half can be implicitly converted to __half* when passed to a kernel expecting __half*.
        pow_kernel<__half><<<num_blocks, threads_per_block>>>(
            reinterpret_cast<const __half*>(base.data_ptr<c10::Half>()),
            reinterpret_cast<const __half*>(exp.data_ptr<c10::Half>()),
