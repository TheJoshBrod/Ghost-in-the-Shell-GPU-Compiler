...
#include <c10/util/Half.h> // Required for c10::Half (though we use __half for device pointers)
...
torch::Tensor launch_pow(torch::Tensor base, torch::Tensor exp) {
    ...
    if (base.scalar_type() == torch::kFloat32) {
        ...
        // Corrected: Use __half directly for data_ptr when the kernel is instantiated with __half.
        // PyTorch's data_ptr can handle __half for kHalf scalar type.
        pow_kernel<__half><<<num_blocks, threads_per_block>>>(
            base.data_ptr<__half>(), exp.data_ptr<__half>(), out.data_ptr<__half>(), n_elements
        );
    } else {
