...
#include <cuda_fp16.h>     // Required for __half type and related functions
#include <string>          // Required for std::string arguments if used, though not for aten::floor
#include <cmath>           // Required for floorf and floor
...
__global__ void floor_kernel(const T* input, T* output, int64_t n_elements) {
    ...
    for (int64_t i = idx; i < n_elements; i += stride) {
        ...
        } else if constexpr (std::is_same_v<T, __half>) { // Use __half for CUDA half-precision type
            output[i] = __float2half(floorf(__half2float(input[i]))); // Use CUDA intrinsics
        } else {
...
torch::Tensor launch_floor(torch::Tensor input) {
    ...
    if (num_blocks > 65535) { // Maximum number of blocks in a grid
        num_blocks = 65535;
    }
    ...
    if (input.scalar_type() == torch::kFloat32) {
        ...
        // c10::Half is a wrapper around __half, so data_ptr<c10::Half>() can be used with __half* in kernel
        floor_kernel<__half><<<num_blocks, threads_per_block>>>(
