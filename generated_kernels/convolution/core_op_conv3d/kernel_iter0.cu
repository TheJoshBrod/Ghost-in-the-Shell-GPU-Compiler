...
__global__ void conv3d_kernel(const T* A, const T* W, const T* B, T* Output,
    ...
    for (int64_t linear_idx = idx; linear_idx < total; linear_idx += stride) {
        ...
        float value = static_cast<float>(B[cout]);
                ...
                for (int64_t kh = 0; kh < KH; ++kh) {
                    for (int64_t kw = 0; kw < KW; ++kw) {
                        ...
                        if (id < D_in && ih < H_in && iw < W_in) {
                            value += static_cast<float>(A[((b * C_in + cin) * D_in + id) * H_in * W_in + ih * W_in + iw]) *
                                     static_cast<float>(W[((cout * C_in + cin) * KD + kd) * KH * KW + kh * KW + kw]);
                        }
                    }
                }
            }
        }
        Output[linear_idx] = static_cast<T>(value);
    }
}
...
torch::Tensor launch_conv3d(torch::Tensor A, torch::Tensor W, torch::Tensor B) {
    ...
    if (A.scalar_type() == torch::kFloat32) {
        conv3d_kernel<float><<<blocks, threads>>>(
            A.data_ptr<float>(), W.data_ptr<float>(), B.data_ptr<float>(), output.data_ptr<float>(),
            B_size, C_in, D_in, H_in, W_in, C_out, KD, KH, KW, D_out, H_out, W_out
        );
    } else if (A.scalar_type() == torch::kFloat64) {
        conv3d_kernel<double><<<blocks, threads>>>(
            A.data_ptr<double>(), W.data_ptr<double>(), B.data_ptr<double>(), output.data_ptr<double>(),
            B_size, C_in, D_in, H_in, W_in, C_out, KD, KH, KW, D_out, H_out, W_out
        );
    } else if (A.scalar_type() == torch::kHalf) {
        conv3d_kernel<c10::Half><<<blocks, threads>>>(
            A.data_ptr<c10::Half>(), W.data_ptr<c10::Half>(), B.data_ptr<c10::Half>(), output.data_ptr<c10::Half>(),
            B_size, C_in, D_in, H_in, W_in, C_out, KD, KH, KW, D_out, H_out, W_out
        );
    } else {
