...
torch::Tensor launch_bmm(std::string equation, torch::TensorList operands) {
    TORCH_CHECK(operands.size() == 2, "Einsum for BMM expects exactly two input tensors.");

    torch::Tensor A_orig = operands[0];
    torch::Tensor B_orig = operands[1];
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    // which now accepts a TensorList for einsum-like operations.
