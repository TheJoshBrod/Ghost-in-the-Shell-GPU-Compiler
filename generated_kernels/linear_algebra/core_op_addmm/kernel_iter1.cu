...
__global__ void addmm_kernel(
    ...
    for (int64_t i = idx; i < total_elements; i += stride) {
        ...
        // Use float for accumulation to maintain precision, then cast back to T
        // For Half, convert to float for computation. For float/double, this is a no-op or promotion.
        float sum_mm = 0.0f;
...
torch::Tensor launch_addmm(
    ...
    torch::Tensor mat1,
    torch::Tensor mat2,
    float beta_d,  // Changed from double to float to match Python's default float type for scalar arguments
    float alpha_d  // Changed from double to float to match Python's default float type for scalar arguments
) {
    // --- Tensor Checks ---
    ...
    if (input.scalar_type() == torch::kFloat32) {
        // beta_d and alpha_d are already float
        ...
        // Convert float scalars to double for the double kernel
        double beta_f = static_cast<double>(beta_d);
        double alpha_f = static_cast<double>(alpha_d);
        ...
        // Convert float scalars to c10::Half for the kernel
